...
if 949 == app.GetDefaultCodePage():
	...
	def MoneyFormat(n):# @METGUIDEwork12
		return "%s" % ('.'.join([i-3<0 and str(n)[:i] or str(n)[i-3:i] for i in range(len(str(n))%3, len(str(n))+1, 3) if i ]))

	if app.METGUIDE_ENABLE_EXP_BOTTLE:
		def CutExperienceString(n) :
			if n <= 0 :
				return "0"
...
def NumberToMoneyStringNew(number):
	...
	result = CutMoneyString(number, 3, 6, "|cffFFD700K|r", result)
	result = CutMoneyString(number, 6, 9, "|cffFFD700M|r", result)
	result = CutMoneyString(number, 9, 12, "|cffFFD700T|r", result)
	result = CutMoneyString(number, 12, 15, "|cffFFD700KT|r", result)
	result = CutMoneyString(number, 15, 18, "|cffFFD700KKT|r", result)
	result = CutMoneyString(number, 18, 20, "|cffFFD700KKKT|r", result)
	# result = result+ "Yang|r"
