...
if app.METGUIDE_ENABLE_ANTIFLAG_TOOLTIP:
	def SplitAntiFlag(desc, limit):
		total_tokens = desc.split()
		line_tokens = []
		line_len = 0
		lines = []
...
class ToolTip(ui.ThinBoard):
	...
	METGUIDE_CONDITION_ANTIFLAG = 0xffFF9999
	CAN_LEVEL_UP_COLOR = 0xff8EC292
	CANNOT_LEVEL_UP_COLOR = DISABLE_COLOR
	NEED_SKILL_POINT_COLOR = 0xff9A9CDB
	if app.METGUIDE_ENABLE_VIEW_ITEM_VNUM:
		ITEM_VNUM_COLOR = 0xffFFFF00
	...
	def AutoAppendTextLine(self, text, color = FONT_COLOR, centerAlign = True):
		...
		# @METGUIDEwork14 BEGIN (moved after the set of the new width)
		if centerAlign:
	...
	if app.METGUIDE_ENABLE_ANTIFLAG_TOOLTIP:
		def AppendAntiFlag(self, desc, color = FONT_COLOR):
			self.__AppendAntiFlag(desc, color)

		def __AppendAntiFlag(self, desc, color=FONT_COLOR):
			lines = SplitAntiFlag(desc, 35)
			if not lines:
				return
...
class ItemToolTip(ToolTip):
	...
	def __init__(self, *args, **kwargs):
		...
		if app.METGUIDE_ENABLE_PRIVATE_SHOP_SEARCH:
	...
	def AddItemData_Offline(self, itemVnum, itemDesc, itemSummary, metinSlot, attrSlot):
		...
		if app.METGUIDE_ENABLE_VIEW_ITEM_VNUM:
			if chr.IsGameMaster(player.GetMainCharacterIndex()):
				self.AppendTextLine("Vnum: %d" % (int(itemVnum)), self.ITEM_VNUM_COLOR)
	...
	def AddItemData(self, itemVnum, metinSlot, attrSlot = 0, flags = 0, unbindTime = 0):
		...
		### Use ###
			...
			if app.METGUIDE_ENABLE_EXP_BOTTLE:
				if constInfo.IS_EXP_BOTTLE(itemVnum):
					leftamount = int(metinSlot[0])
					maxvalue = int(constInfo.GetExpBottleMax(itemVnum))
					
					self.AppendSpace(5)
					self.AppendTextLine(localeInfo.TOOLTIP_EXP_BOTTLE + ": " + "%s/%s" % (localeInfo.CutExperienceString(leftamount), localeInfo.CutExperienceString(maxvalue)), self.SPECIAL_POSITIVE_COLOR)
		...
		if app.METGUIDE_ENABLE_VIEW_ITEM_VNUM:
			if chr.IsGameMaster(player.GetMainCharacterIndex()):
				self.AppendTextLine(localeInfo.ITEM_VNUM_TOOLTIP % (int(itemVnum)), self.ITEM_VNUM_COLOR)
	...
	if app.METGUIDE_ENABLE_ANTIFLAG_TOOLTIP:
		def AppendAntiflagInformation(self):
			antiFlagDict = {
				localeInfo.METGUIDE_TOOLTIP_ANTIFLAG_DROP: item.IsAntiFlag(item.ITEM_ANTIFLAG_DROP),
				localeInfo.METGUIDE_TOOLTIP_ANTIFLAG_SELL: item.IsAntiFlag(item.ITEM_ANTIFLAG_SELL),
				localeInfo.METGUIDE_TOOLTIP_ANTIFLAG_GIVE: item.IsAntiFlag(item.ITEM_ANTIFLAG_GIVE),
				localeInfo.METGUIDE_TOOLTIP_ANTIFLAG_PKDROP: item.IsAntiFlag(item.ITEM_ANTIFLAG_PKDROP),
				localeInfo.METGUIDE_TOOLTIP_ANTIFLAG_STACK: item.IsAntiFlag(item.ITEM_ANTIFLAG_STACK),
				localeInfo.METGUIDE_TOOLTIP_ANTIFLAG_MYSHOP: item.IsAntiFlag(item.ITEM_ANTIFLAG_MYSHOP),
				#localeInfo.METGUIDE_TOOLTIP_ANTIFLAG_SAFEBOX: item.IsAntiFlag(item.ITEM_ANTIFLAG_SAFEBOX),
			}
	
			antiFlagNames = [name for name, flag in antiFlagDict.items() if flag]
			if antiFlagNames:
				self.AppendSpace(5)

				notPossibleLine = self.AppendTextLine('{}'.format(localeInfo.METGUIDE_NOT_POSSIBLE), self.METGUIDE_CONDITION_ANTIFLAG)
				notPossibleLine.SetFeather()

				textLine = self.AppendTextLine(', '.join(antiFlagNames), self.METGUIDE_CONDITION_ANTIFLAG)
				textLine.SetFeather()
