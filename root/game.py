...
# DATA_TIME_METGUIDE
import time
# END_DATA_TIME_METGUIDE
if app.METGUIDE_ENABLE_TRIVIA_EVENT:
	import uiTriviaPanel
...
# END_OFFLINE_SHOPS_METGUIDE
		...
		#if app.METGUIDE_ENABLE_EVENT_MANAGER:
		...
		if app.METGUIDE_ENABLE_TRIVIA_EVENT:
			self.triviapanel = uiTriviaPanel.TriviaPanelWindow()
...
# DATA_TIME_METGUIDE
		...
		self.timeLine.SetFontColor(  1.0, 1.0,   1.0)
		self.timeLine.SetPosition((wndMgr.GetScreenWidth() - 130) / 2, 176)

		self.dateLine = ui.TextLine()
		self.dateLine.SetFontName(localeInfo.UI_DEF_FONT_LARGE)
		self.dateLine.SetFontColor(  1.0, 1.0,   1.0)
		self.dateLine.SetPosition((wndMgr.GetScreenWidth() - 130) / 2, 190)
# END_DATA_TIME_METGUIDE
...
# DATA_TIME_METGUIDE
		...
		self.dateLine.SetWindowHorizontalAlignCenter()
		self.dateLine.SetHorizontalAlignCenter()
		self.dateLine.SetFeather()
		self.dateLine.SetOutline()
		self.dateLine.Show()
# END_DATA_TIME_METGUIDE
	...
	## TargetBoard
	if app.METGUIDE_ENABLE_TARGET_HP_SYSTEM:
	...
	def OnPickMoney(self, money):
		chat.AppendChat(chat.CHAT_TYPE_INFO, localeInfo.GAME_PICK_MONEY % localeInfo.MoneyFormat(money)) # @METGUIDEwork12 "locale_game GAME_PICK_MONEY = %d -> %s"
...
# DATA_TIME_METGUIDE
		self.timeLine.SetText(time.strftime("%H:%M:%S"))
		self.dateLine.SetText(time.strftime("%a, %d %b %Y"))
# END_DATA_TIME_METGUIDE
...
# END_OFFLINE_SHOPS_METGUIDE
	...
	def BINARY_Highlight_Item(self, inven_type, inven_pos):
		# @METGUIDEwork03 (+if self.interface:)
		if self.interface:
...
# END_OFFLINE_SHOPS_METGUIDE
		...
		if app.METGUIDE_ENABLE_MOVE_CHANNEL:
			serverCommandList["server_info"] = self.__SeverInfo

		if app.METGUIDE_ENABLE_TRIVIA_EVENT:
			serverCommandList["OpenTriviaPanel"] = self.__OpenTriviaPanel
	...
	if app.METGUIDE_ENABLE_CUBE_RENEWAL_WORLDARD:
...
# END_OFFLINE_SHOPS_METGUIDE
	...
	if app.METGUIDE_ENABLE_MAINTENANCE_SYSTEM:
	...
	if app.METGUIDE_ENABLE_TRIVIA_EVENT:
		def __OpenTriviaPanel(self):
			if chr.IsGameMaster(player.GetMainCharacterIndex()):
				self.triviapanel.OpenWindow()
