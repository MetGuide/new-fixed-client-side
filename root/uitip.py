...
class TipBoard(ui.Bar):
	...
	def SetTip(self, text):
		...
		curTime = app.GetTime()
		self.tipList.append((curTime, self.DeleteColors(text)))
		self.__RefreshBoard()
	...
	def DeleteColors(self, text):
		import re
		searchtext = re.search('\|c([a-zA-Z0-9]{0,8})', text)
		if searchtext:
			text = text.replace(searchtext.group(0),'')
			text = text.replace('|r','')
			text = text.replace('|h','')
			text = text.replace('|H','')
			text = text.replace('|R','')
...
class BigBoard(ui.Bar):
	...
	def SetTip(self, text):
		...
		curTime = app.GetTime()
		self.__AppendText(curTime, self.DeleteColors(text))
		self.__RefreshBoard()
