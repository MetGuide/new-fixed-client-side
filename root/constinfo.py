import app
PM_GM_IGNORE = ["sall", "sal", "esti", "esti?", "am o problema", "salut", "buna ziua", "buna"] 
GM_LIST_DICT = ["METGUIDE", "[DEV]wawaa"]
...
if app.METGUIDE_ENABLE_EXP_BOTTLE:
	EXP_BOTTLE_DATA = [[71800, 5000], [71801, 50000], [71802, 100000]] #Item vnum, exp value
	def IS_EXP_BOTTLE(itemVnum):
		for i in range(len(EXP_BOTTLE_DATA)):
			if itemVnum == EXP_BOTTLE_DATA[i][0]:
				return 1
		return 0
		
	def GetExpBottleMax(itemVnum):
		for i in range(len(EXP_BOTTLE_DATA)):
			if itemVnum == EXP_BOTTLE_DATA[i][0]:
				return int(EXP_BOTTLE_DATA[i][1])
		return 0
