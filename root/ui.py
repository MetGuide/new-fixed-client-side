...
class EditLine(TextLine):
	...
	def OnPressEscapeKey(self):
		if self.IsFocus(): #@METGUIDEwork15 (IsFocus check)
			self.eventEscape()
			return True
		return False
...
# END_OFFLINE_SHOPS_METGUIDE
	...
	def SetItemSlot(self, renderingSlotNumber, ItemIndex, ItemCount = 0, diffuseColor = (1.0, 1.0, 1.0, 1.0),id=0):# OFFLINE_SHOPS_METGUIDE
...
class ListBox(Window):
	...
	def SelectItem(self, line):
		...
		# if line == self.selectedLine:
			# return
		if not app.METGUIDE_ENABLE_PRIVATE_SHOP_SEARCH:
			if line == self.selectedLine:
				return
