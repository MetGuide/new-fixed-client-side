...
import chr
import constInfo
if app.METGUIDE_ENABLE_WHISPER_RENEWAL:
	import time
	import whisper
	b_name = "Typing."
...
class WhisperDialog(ui.ScriptWindow):
	...
	def SendWhisper(self):
		...
		if textLength > 0:
			...
			if app.METGUIDE_ENABLE_LINK_IN_CHAT:
			...
			if self.targetName in constInfo.GM_LIST_DICT and text in constInfo.PM_GM_IGNORE:
				chat.AppendWhisper(chat.WHISPER_TYPE_CHAT, self.targetName, "|cFF00FF00[i]|r |cFFFF8040Te rugam sa adresezi direct intrebarea! Vei primi un raspuns in cel mai rapid timp posibil! Te rugam sa nu faci spam, daca nu primesti raspuns, trimite un alt mesaj peste 5 - 10 minute!")
				chat.AppendWhisper(chat.WHISPER_TYPE_CHAT, self.targetName, "|cFF00FF00[i]|r |cFFFF8040Nu uita, poti gasi toate informatile si pe Discord!")
				return
