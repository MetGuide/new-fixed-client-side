...
import constInfo
import background
if app.METGUIDE_ENABLE_TELEPORT_MINIMAP_GM:
	import chr
...
class MiniMap(ui.ScriptWindow):
	...
	def __LoadWindow(self):
		...
		if miniMap.IsAtlas():
			self.AtlasShowButton.SetEvent(ui.__mem_func__(self.ToggleAtlasWindow)) # @METGUIDEwork11 ShowAtlas + transfer

		self.RefreshTooltipPosition()# =transfer

# Am mutat logica de poziționare a tooltip-urilor într-o metodă separată
#transfer MiniMapShowButton
#transfer MiniMapHideButton
#transfer ScaleUpButton
#transfer ScaleDownButton
#transfer AtlasShowButton

		self.ShowMiniMap()

	def RefreshTooltipPosition(self): #transfer here end
